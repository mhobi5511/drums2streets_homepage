...
function FloatingBookingButton({ visible }: { visible: boolean }) {
  return (
    <Link
      href="/#kontakt"
      dataFloatingCta
      className={`fixed bottom-[calc(env(safe-area-inset-bottom)+0.75rem)] right-20 z-50 rounded-md border border-white/10 bg-[#8f6b32] px-5 py-4 text-xs font-black uppercase text-white shadow-2xl shadow-black/50 transition hover:bg-[#b99b5d] sm:bottom-6 sm:right-24 ${
        visible
...
function ScrollToTopButton({ visible }: { visible: boolean }) {
  ...
  return (
    <button
      ...
      className={`fixed bottom-[calc(env(safe-area-inset-bottom)+0.75rem)] right-5 z-50 grid h-12 w-12 place-items-center rounded-md border border-white/10 bg-black/75 text-lg font-black text-white shadow-2xl shadow-black/50 transition hover:border-[#b99b5d] hover:text-[#ead8a8] sm:bottom-6 sm:right-6 ${
        visible
          ? 'translate-y-0 opacity-100'
          : 'pointer-events-none translate-y-4 opacity-0 sm:pointer-events-auto sm:translate-y-0 sm:opacity-100'
      }`}
